    ...
    try:
        with job_lock:
            ...
            for job_id, job_data in jobs_to_remove:
                ...
                # Clean up non-business screenshots for this job (preserve business screenshots)
                try:
                    import os
        ...
        try:
            ...
            try:
                ...
                for idx, result in enumerate(results):
                    ...
                    if existing_lead:
                        duplicate_count += 1
                        # Update screenshot path if new screenshot was captured
                        screenshot_path = result.get('screenshot_filename')
                        if screenshot_path and not existing_lead.screenshot_path:
                            existing_lead.screenshot_path = screenshot_path
                            existing_lead.updated_at = datetime.utcnow()
                            db.commit()
                            add_job_log(job_id, f"Duplicate: {business_name} - updated with screenshot")
                        else:
                    ...
                    if name_location_duplicate:
                        ...
                        # Update screenshot path if new screenshot was captured
                        screenshot_path = result.get('screenshot_filename')
                        if screenshot_path and not name_location_duplicate.screenshot_path:
                            name_location_duplicate.screenshot_path = screenshot_path
                            name_location_duplicate.updated_at = datetime.utcnow()
                            db.commit()
                            add_job_log(job_id, f"Duplicate: {business_name} - updated with screenshot")
                        else:
